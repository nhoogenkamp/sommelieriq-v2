...
def webhook_received():
    ...
    # When any update for example user cancels subscription but still has some days left on it.
        ...
        if canceled_at is not None:
            ...
            try:
                cancel_sql = """ UPDATE companies SET subscription_status = %s WHERE stripe_subscription_id = %s """
                cancel_values = ("canceling", stripe_subscription_id)
                cursor.execute( cancel_sql, cancel_values)

                con.commit()

                print("Subscription canceling:", stripe_subscription_id)
